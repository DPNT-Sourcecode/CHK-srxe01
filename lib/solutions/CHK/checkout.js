...
module.exports = function (skus) {
    ...
    return Object.keys(cartSkuCount).reduce((acc, sku) => {
        ...
        if ('free' in skuPricing) {
            ...
            console.dir(freeLevels)
            let validFree = null
            freeLevels.map((level) => {
                if (cartCount >= level) {
                    console.log('got a freebie')
                    const freeSku = skuPricing.free[level.toString(10)]
                    console.log('freeSKu ' + freeSku + ' ' + cartSkuCount[freeSku])
                    if (cartSkuCount[freeSku] > 0) priceDiscount -= skuPriceTable[freeSku].price
                }
            })
        }
        if ('multi' in skuPricing) {
            ...
            // console.log(`acc: ${acc} ${cartCount} ${skuPricing.price} ${priceDiscount}`)
